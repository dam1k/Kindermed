          ...
          <ul className="flex max-[1400px]:hidden h-[44px] gap-[15px]">
            {navLinks.map((link: navLink) => {
              return (
                ...
                >
                  <span>{link.text}</span>
                  {link.href === "/vaccinare" && <span className={`${link.href === activeLink ? "bg-white text-[#00AAF1]" : "text-white bg-[#00AAF1]"} py-[4px] px-[7px] text-[12px] leading-[12.6px] rounded-[8px]`}>NEW</span>}
                </li>
              );
            })}
          </ul>

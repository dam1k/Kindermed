...
function MobileMenu({working, day, setMenuOpen, setAppointmentOpen, handleLinkClick}:IMobileMenuProps) {
    ...
    return (
        ...
        key="menu_overlay">
            <div ref={menuRef} className="fixed overflow-scroll bg-[#FFFFFF] h-[75vh] bottom-0 left-0 w-screen p-[15px] pb-[75px] rounded-tr-[25px] rounded-tl-[25px]">
                <div className="flex items-center justify-between px-[5px] pt-[5px]">
                    <h3 className="text-[18px] leading-[18.9px] uppercase">Navigare</h3>
                    <button className="!bg-[#3E404D]/[0.08] mt-[5px] rounded-[10px] w-[25px] h-[25px] flex items-center justify-center"
                    onClick={handleClose}>
                    ...
                    {navLinks.map((link: navLink) => {
                        return (
                            <li key={link.href}
                                ...
                                <span>{link.text}</span>
                                {link.href === "/vaccinare" && <span className="leading-[9px] uppercase flex items-center justify-center px-[6px] text-[10px] text-white bg-[#00AAF1] h-[17px] rounded-[7px]">new</span>}
                                {/*{link.href === "#team" && <div*/}
